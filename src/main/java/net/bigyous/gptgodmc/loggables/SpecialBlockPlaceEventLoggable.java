...
public class SpecialBlockPlaceEventLoggable extends BaseLoggable {
    ...
    private List<Tag<Material>> specialGroups = List.of(MaterialSetTag.ALL_SIGNS, MaterialSetTag.DOORS,
            MaterialSetTag.BEDS, MaterialSetTag.CANDLES, MaterialSetTag.ITEMS_BOATS);
    ...
    private String getlocation(Player player) {
        ...
        if (StructureManager.getStructureProximityData(player.getLocation()) != null) {
            ...
            if (StructureManager.getStructureProximityData(player.getLocation()).getDistance() >= 10) {
                return String.format("%d blocks away from %s",
                        StructureManager.getStructureProximityData(player.getLocation()).getDistance(),
                        StructureManager.getStructureProximityData(player.getLocation()).getStructure());
            }
        }
        return "";
    }
    ...
    public boolean combine(Loggable l) {
        if (!(l instanceof SpecialBlockPlaceEventLoggable) || !isValid)
            return false;
